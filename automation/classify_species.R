...
## Owen's paths
to.data.owen <- "/Users/owenpetchey/Desktop/franco.test.vids/data/6 - Classification/"

## Frank's paths
to.data.frank <- "C:/Users/Frank/Documents/PhD/Programming/franco/data/6 - Classification/"
...
fit_move <- princomp(species_subsets[, c(4,5,6,7,8,9)], cor=TRUE)
...
predict_visual <- sqldf("select p.id, p.predict_spec, t.*
...
dir.create(sub("2 - trajectory data/",prediction.folder,path))
...
trajectory.data_tmp <- subset(trajectory.data,file == file_names[i])
  j<- 0
    while(j < max(trajectory.data$frame)+1){
      jpeg(paste(sub(trajectory.data.folder,prediction.folder,path),as.character(file_names[i]),"/","frame_",j,".jpg",sep=""), width = as.numeric(width), height = as.numeric(height), quality = 100)
...
text[grep("overlay_input = ", text)] <- paste("overlay_input = ","'", sub(trajectory.data.folder,prediction.folder,path),"';", sep = "")
text[grep("overlay_output = ", text)] <- paste("overlay_output = ","'", sub(trajectory.data.folder,prediction.folder2,path),"';", sep = "")
